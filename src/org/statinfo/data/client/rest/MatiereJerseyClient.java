...
/**
 * Jersey REST client generated for REST resource:MatiereResource [matiere]<br>
 * USAGE:
 * <pre>
 *        MatiereJerseyClient client = new MatiereJerseyClient();
 ...
 * @author boubad
 */
public class MatiereJerseyClient {
    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8888/InfoStat-war/webresources";

    public MatiereJerseyClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(MatiereDTO.class);
         config.getClasses().add(MatiereDTO[].class);
        client = Client.create(config);
        webResource = client.resource(BASE_URI).path("matiere");
    }
    ...
    public <T> T findByUniteSigle_XML(Class<T> responseType, String id, String sigle) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (id != null) {
    ...
    public <T> T findByUniteSigle_JSON(Class<T> responseType,String id,String sigle) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (id != null) {
            resource = resource.queryParam("id", id);
        }
    ...
    public <T> T findByUnite_XML(Class<T> responseType, String id) throws UniformInterfaceException {
        WebResource resource = webResource;
        resource = resource.path(java.text.MessageFormat.format("unite/{0}", new Object[]{id}));
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_XML).get(responseType);
    }

    public <T> T findByUnite_JSON(Class<T> responseType, String id) throws UniformInterfaceException {
        WebResource resource = webResource;
        resource = resource.path(java.text.MessageFormat.format("unite/{0}", new Object[]{id}));
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_JSON).get(responseType);
    }
