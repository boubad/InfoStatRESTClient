...
/**
 * Jersey REST client generated for REST resource:GroupeResource [groupe]<br>
 * USAGE:
 * <pre>
 *        GroupeJerseyClient client = new GroupeJerseyClient();
 ...
 * @author boubad
 */
public class GroupeJerseyClient {
    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8888/InfoStat-war/webresources";

    public GroupeJerseyClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(GroupeDTO.class);
        config.getClasses().add(GroupeDTO[].class);
        client = Client.create(config);
        webResource = client.resource(BASE_URI).path("groupe");
    }
     public GroupeJerseyClient(String baseURI) {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(GroupeDTO.class);
        config.getClasses().add(GroupeDTO[].class);
        client = Client.create(config);
        webResource = client.resource(baseURI).path("groupe");
    }
