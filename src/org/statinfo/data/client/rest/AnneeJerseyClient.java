...
/**
 * Jersey REST client generated for REST resource:AnneeResource [annee]<br>
 * USAGE:
 * <pre>
 *        AnneeJerseyClient client = new AnneeJerseyClient();
 ...
 * @author boubad
 */
public class AnneeJerseyClient {
    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8888/InfoStat-war/webresources";

    public AnneeJerseyClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(AnneeDTO.class);
        config.getClasses().add(AnneeDTO[].class);
        client = Client.create(config);
        webResource = client.resource(BASE_URI).path("annee");
    }
    public AnneeJerseyClient(String baseURI) {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(AnneeDTO.class);
        config.getClasses().add(AnneeDTO[].class);
        client = Client.create(config);
        webResource = client.resource(baseURI).path("annee");
    }
