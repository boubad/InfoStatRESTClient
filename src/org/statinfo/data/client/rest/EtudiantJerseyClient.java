...
/**
 * Jersey REST client generated for REST resource:EtudiantResource
 * [etudiant]<br>
 * USAGE:
 * <pre>
 *        EtudiantJerseyClient client = new EtudiantJerseyClient();
 ...
 * @author boubad
 */
public class EtudiantJerseyClient {
    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8888/InfoStat-war/webresources";

    public EtudiantJerseyClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(EtudiantDTO.class);
        config.getClasses().add(EtudiantDTO[].class);
        client = Client.create(config);
        webResource = client.resource(BASE_URI).path("etudiant");
    }
    public EtudiantJerseyClient(String baseURI) {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(EtudiantDTO.class);
        config.getClasses().add(EtudiantDTO[].class);
        client = Client.create(config);
        webResource = client.resource(baseURI).path("etudiant");
    }
    ...
    public <T> T findPhoto(Class<T> responseType, String id) throws UniformInterfaceException {
        WebResource resource = webResource;
        resource = resource.path(java.text.MessageFormat.format("photo/{0}", new Object[]{id}));
    ...
    public <T> T findByLastnameFirstname_XML(Class<T> responseType, 
            String lastname, String firstname) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (lastname != null) {
            resource = resource.queryParam("lastname", lastname);
        }
         if (firstname != null) {
            resource = resource.queryParam("firstname", firstname);
        }
        resource = resource.path("user");
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_XML).get(responseType);
    }

    public <T> T findByLastnameFirstname_JSON(Class<T> responseType, 
            String lastname,String firstname) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (lastname != null) {
            resource = resource.queryParam("lastname", lastname);
