...
/**
 * Jersey REST client generated for REST resource:EnseignantResource
 * [enseignant]<br>
 * USAGE:
 * <pre>
 *        EnseignantJerseyClient client = new EnseignantJerseyClient();
 ...
 * @author boubad
 */
public class EnseignantJerseyClient {

    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8888/InfoStat-war/webresources";

    public EnseignantJerseyClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(EnseignantDTO.class);
        config.getClasses().add(EnseignantDTO[].class);
        client = Client.create(config);
        webResource = client.resource(BASE_URI).path("enseignant");
    }

    public EnseignantJerseyClient(String baseURI) {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(EnseignantDTO.class);
        config.getClasses().add(EnseignantDTO[].class);
        client = Client.create(config);
        webResource = client.resource(baseURI).path("enseignant");
    }
    ...
    public <T> T findByUsername_XML(Class<T> responseType, String username) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (username != null) {
    ...
    public <T> T findByUsername_JSON(Class<T> responseType, String username) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (username != null) {
            resource = resource.queryParam("username", username);
        }
