...
/**
 * Jersey REST client generated for REST resource:GroupeEventResource
 * [grpevt]<br>
 * USAGE:
 * <pre>
 *        GroupeEventJerseyClient client = new GroupeEventJerseyClient();
 ...
 * @author boubad
 */
public class GroupeEventJerseyClient {

    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8888/InfoStat-war/webresources";

    public GroupeEventJerseyClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(GroupeEventDTO.class);
        config.getClasses().add(GroupeEventDTO[].class);
        client = Client.create(config);
        webResource = client.resource(BASE_URI).path("grpevt");
    }

    public GroupeEventJerseyClient(String baseURI) {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(GroupeEventDTO.class);
        config.getClasses().add(GroupeEventDTO[].class);
        client = Client.create(config);
        webResource = client.resource(baseURI).path("grpevt");
    }
    ...
    public <T> T findData_XML(Class<T> responseType, String groupeid, String matiereid, String enseignantid, String semestreid) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (groupeid != null) {
