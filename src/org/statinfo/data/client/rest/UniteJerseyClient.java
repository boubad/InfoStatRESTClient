...
/**
 * Jersey REST client generated for REST resource:UniteResource [unite]<br>
 * USAGE:
 * <pre>
 *        UniteJerseyClient client = new UniteJerseyClient();
 ...
 * @author boubad
 */
public class UniteJerseyClient {

    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8888/InfoStat-war/webresources";

    public UniteJerseyClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(UniteDTO.class);
        config.getClasses().add(UniteDTO[].class);
        client = Client.create(config);
        webResource = client.resource(BASE_URI).path("unite");
    }

    public UniteJerseyClient(String baseURI) {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        config.getClasses().add(UniteDTO.class);
        config.getClasses().add(UniteDTO[].class);
        client = Client.create(config);
        webResource = client.resource(baseURI).path("unite");
    }
    ...
    public <T> T findByDepartementSigle_XML(Class<T> responseType, String depId,
            String sigle) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (depId != null) {
            resource = resource.queryParam("id", depId);
        }
        if (sigle != null) {
    ...
    public <T> T findByDepartementSigle_JSON(Class<T> responseType, String depId,
            String sigle) throws UniformInterfaceException {
        WebResource resource = webResource;
        if (depId != null) {
            resource = resource.queryParam("id", depId);
